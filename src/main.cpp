...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
	{
		...
		if (capture->get(cv::CAP_OPENNI_DEPTH_GENERATOR_PRESENT))
		{
			std::cout << "\nDepth generator output mode:" << std::endl
					<< "FRAME_WIDTH      "
					<< capture->get(cv::CAP_PROP_FRAME_WIDTH) << std::endl
					<< "FRAME_HEIGHT     "
					<< capture->get(cv::CAP_PROP_FRAME_HEIGHT) << std::endl
					<< "FRAME_MAX_DEPTH  "
					<< capture->get(cv::CAP_PROP_OPENNI_FRAME_MAX_DEPTH)
					<< " mm" << std::endl << "FPS              "
					<< capture->get(cv::CAP_PROP_FPS) << std::endl
					<< "REGISTRATION     "
					<< capture->get(cv::CAP_PROP_OPENNI_REGISTRATION)
					<< std::endl << "IMG_GEN          "
					<< capture->get(cv::CAP_OPENNI_IMAGE_GENERATOR_PRESENT)
					<< std::endl;
		}
		else
		{
	...
	//Code for initializing stuff for reading images from files, if no kinect is used
	else
	{
		dataFolder = std::string(argv[1]) + "/";
		// load file names
		assocFile = dataFolder + "rgbd_assoc.txt";
		if (!loadAssoc(assocFile, filesDepth, filesColor, timestampsDepth,
				timestampsColor))
		{
	...
	// code for visualization initialization
	cv::viz::Viz3d mainWindow("Odometry");
	mainWindow.setWindowSize(cv::Size(1600, 900));
	cv::Matx33f vizK(K(0, 0), K(0, 1), K(0, 2), K(1, 0), K(1, 1), K(1, 2),
			K(2, 0), K(2, 1), K(2, 2));
	...
	for (size_t i = 1; i < numFrames && (maxFrames < 0 || i < maxFrames); ++i)
	{
		...
		//enable interrupting by keypress
		if (cv::waitKey(1) >= 0)
			break;
		...
		dvo.buildPyramid(depthCur, grayCur, depthCurGPUPyramid,
				grayCurGPUPyramid);
		...
		dvo.align(depthRefGPUPyramid, grayRefGPUPyramid, depthCurGPUPyramid,
				grayCurGPUPyramid, relPose);
		...
		float data[] =
		{ absPose(0, 0), absPose(0, 1), absPose(0, 2), absPose(0, 3), absPose(1,
				0), absPose(1, 1), absPose(1, 2), absPose(1, 3), absPose(2, 0),
				absPose(2, 1), absPose(2, 2), absPose(2, 3), absPose(3, 0),
				absPose(3, 1), absPose(3, 2), absPose(3, 3) };

		//visualize
		vizPoses.push_back(cv::Affine3f(data));
		mainWindow.showWidget("cameras_line",
				cv::viz::WTrajectory(vizPoses, cv::viz::WTrajectory::PATH, 0.1,
						cv::viz::Color::green()));
		mainWindow.showWidget("cameras_frustums",
				cv::viz::WTrajectoryFrustums(vizPoses, vizK, 0.1,
						cv::viz::Color::red()));
		...
		for (int x = 0; x < w; x++)
		{
			for (int y = 0; y < h; y++)
			{
				...
				{
					points.at < cv::Vec3f > (y, x) = cv::Vec3f(dNaN, dNaN,
							dNaN);
				}
			}
		}
	...
	std::cout << "average runtime: " << (runtimeAvg / framesProcessed) * 1000.0
			<< " ms" << std::endl;
